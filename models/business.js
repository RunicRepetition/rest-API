...
async function getBusinessById(id) {
    const db = getDbReference()
    const collection = db.collection('businesses')
    const img_bucket = new GridFSBucket(db, { bucketName: "images" })
    if (!ObjectId.isValid(id)) {
        return null
    } else {
        const results = { 
           business: await collection.find({_id: new ObjectId(id)}).toArray(),
           images: await img_bucket.find({"metadata.businessId": id}).toArray()
        }
        return results
    }
}
